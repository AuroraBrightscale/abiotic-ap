...
function apClient.connect(server, slot, password)
    local client = {}
    function client.on_socket_connected()
        print("Socket connected")
    end

    function client.on_socket_error(msg)
        print("Socket error: " .. msg)
    end

    function client.on_socket_disconnected()
        print("Socket disconnected")
    end

    function client.on_room_info()
        print("Room info")
        ap:ConnectSlot(slot, password, items_handling, { "Lua-APClientPP" }, client_version)
    end

    function client.on_slot_connected(slot_data)
    ...
    function client.on_slot_refused(reasons)
        print("Slot refused: " .. table.concat(reasons, ", "))
    end

    function client.on_items_received(items)
    ...
    function client.on_location_info(items)
    ...
    function client.on_location_checked(locations)
        print("Locations checked:" .. table.concat(locations, ", "))
        print("Checked locations: " .. table.concat(ap.checked_locations, ", "))
    end

    function client.on_data_package_changed(data_package)
        print("Data package changed:")
        print(data_package)
    end

    function client.on_print(msg)
        print(msg)
    end

    function client.on_print_json(msg, extra)
    ...
    function client.on_bounced(bounce)
        print("Bounced:")
        print(bounce)
    end

    function client.on_retrieved(map, keys, extra)
        print("Retrieved:")
    ...
    function client.on_set_reply(message)
        print("Set Reply:")
    ...
    ap:set_socket_connected_handler(client.on_socket_connected)
    ap:set_socket_error_handler(client.on_socket_error)
    ap:set_socket_disconnected_handler(client.on_socket_disconnected)
    ap:set_room_info_handler(client.on_room_info)
    ap:set_slot_connected_handler(client.on_slot_connected)
    ap:set_slot_refused_handler(client.on_slot_refused)
    ap:set_items_received_handler(client.on_items_received)
    ap:set_location_info_handler(client.on_location_info)
    ap:set_location_checked_handler(client.on_location_checked)
    ap:set_data_package_changed_handler(client.on_data_package_changed)
    ap:set_print_handler(client.on_print)
    ap:set_print_json_handler(client.on_print_json)
    ap:set_bounced_handler(client.on_bounced)
    ap:set_retrieved_handler(client.on_retrieved)
    ap:set_set_reply_handler(client.on_set_reply)
end

-- apClient.connect(host, slot, password)
return apClient
